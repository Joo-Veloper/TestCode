...
* **구조**: 프레젠테이션 → 서비스 → 도메인 → 리포지토리(DB) 순으로 수직 계층 구성
* **문제점**:

  * 하위 계층(특히 DB)에 의존하게 되며, **DB 중심의 사고방식**을 갖게 됨
  * **상향식 접근 시**: JPA(Entity)부터 설계하게 되는 경향
  * **하향식 접근 시**: 프레임워크(Spring, MVC 등)부터 고려하게 됨
* → **비즈니스 로직 중심 설계가 어려워짐**
...
* **구조**: 도메인(핵심 로직)을 중심으로, 포트(인터페이스)와 어댑터(프레임워크, DB, UI 등)가 바깥에 위치
* **장점**:

  * **상향식 접근**이 자연스러움
  * 프레임워크, DB, UI 등 외부 요소를 독립적으로 다룰 수 있어 **테스트 용이성 및 유지보수성 향상**
  * 도메인 로직을 중심으로 사고할 수 있음
...
* 테스트하기 어려운 요소(GUI, DB 등)를 **분리된 험블 객체**로 따로 분리
* 본질적인 로직(비즈니스 로직)과 험블 객체(테스트 어려운 요소)를 명확하게 구분

### 🔸 왜 구분해야 하는가?

- "라이브러리를 바꾼다고 계산 로직이 변경되면 안 된다"
- "프레임워크를 바꾼다고 계산 로직이 변경되면 안 된다"
- "심지어 언어를 바꿔도 계산 로직은 유지되어야 한다"

* **설계의 핵심은 의존성의 방향 제어와 고립성 유지**
* 험블 객체는 외부 의존성이 크므로, **테스트에서 배제**하거나 **Mocking 대상으로 삼음**

---

## ✅ 헥사고날 아키텍처에서의 어댑터와 유스케이스

### 🔷 어댑터(Adapter)의 역할

어댑터는 외부 시스템(웹, DB, 메시지 등)과 내부 애플리케이션(도메인/유스케이스) 사이의 **입·출력 연결 역할**을 합니다.

---

### 🔸 1. **Input 어댑터 (예: 웹 어댑터, Controller)**

#### 📌 주요 역할:

1. **HTTP 요청을 자바 객체로 변환** (ex. JSON → DTO)
2. **권한 검사**
3. **입력 유효성 검증**
4. 웹 DTO를 → **유스케이스 입력 모델로 변환**
5. **유스케이스 호출**
6. **유스케이스 결과를 HTTP 응답 형식으로 변환**
7. **HTTP 응답 반환**

> ⚠️ **주의사항**
> 웹 어댑터에서 유효성 검증을 중복 구현할 필요는 없습니다.
> 다만, “웹 입력 DTO → 유스케이스 입력 모델”로 변환이 가능한지만 검증하면 충분합니다.

---

### 🔸 2. **Output 어댑터 (예: JPA 어댑터, Repository)**

#### 📌 주요 역할:

1. 유스케이스 또는 도메인으로부터 **입력**을 받음
2. 이를 DB 포맷(JPA Entity)로 매핑
3. **DB에 저장하거나 조회**
4. 결과를 **도메인 모델 또는 애플리케이션 포맷으로 변환**
5. 유스케이스 또는 도메인으로 **출력 반환**

> 💡 자바에서는 보통 JPA를 사용하기 때문에, **입력 모델을 JPA 엔티티로 매핑**하고,
> **DB 결과를 도메인 모델 또는 DTO로 다시 변환**하는 구조가 일반적입니다.

---

## ✅ 유스케이스의 역할

* **도메인 규칙을 바탕으로 애플리케이션의 흐름을 정의**
* 비즈니스 로직을 **조합하고 실행**하는 단위
* 외부 의존성(DB, UI 등)에 대해 무지(無知)해야 하며, 순수한 자바 코드로 유지됨
...
| 요소         | 역할 요약                               |
| ---------- | ----------------------------------- |
| Input 어댑터  | 외부 요청(HTTP 등)을 유스케이스 입력 모델로 변환 후 호출 |
| 유스케이스      | 비즈니스 로직 실행 (도메인 조합 및 흐름 제어)         |
| Output 어댑터 | 유스케이스 요청을 DB 형식(JPA 등)으로 변환하고 처리    |
