...
### 1. `@NotNull` 🚫
- **목적**: 필드가 `null`이 아니어야 한다는 제약을 설정합니다.
- **특징**: `null`인 값만 체크하며, 빈 문자열(`""`)은 유효하다고 판단합니다.
...
### 2. `@NotEmpty` 🏷️
- **목적**: 필드가 `null`이거나 빈 값(`""`)일 수 없다는 제약을 설정합니다.
- **특징**: `null` 및 빈 문자열(`""`)을 모두 거부합니다. 컬렉션의 경우, 요소가 하나 이상이어야 합니다.
- **사용 예**: 텍스트 필드에 최소한의 데이터가 필요할 때 사용합니다.
...
### 3. `@NotBlank` 📝
- **목적**: 필드가 `null`이거나 공백만 있는 문자열이 아니어야 한다는 제약을 설정합니다.
- **특징**: `null`, 빈 문자열(`""`), 공백만 있는 문자열(`" "`)을 모두 거부합니다. 즉, "공백이 아닌 실제 텍스트"가 있어야 합니다.
- **사용 예**: 텍스트 필드에서 의미 있는 값이 필요할 때 사용합니다.
