...
### 1. **트랜잭션을 사용한 테스트 (`@Transactional`)의 장점**
테스트 클래스 위에 `@Transactional`을 추가하면 각 테스트가 끝난 후 자동으로 **롤백**되기 때문에,  
테스트 실행 후 DB 상태가 항상 초기 상태로 유지됩니다.

✅ **장점:**
- 별도의 정리 코드 (`@AfterEach`)가 필요 없음 → `deleteAllInBatch()` 안 해도 됨
- 테스트 간 데이터가 섞일 걱정이 없음 → 독립적인 테스트 가능

---

### 2. **트랜잭션을 사용하지 않고 `@AfterEach`를 사용하는 이유**
테스트 클래스에 `@Transactional`을 붙이면, `OrderService` 내부에 `@Transactional`이 있든 없든 상관없이 **테스트가 정상적으로 동작**합니다.  
왜냐하면 테스트 자체가 하나의 트랜잭션으로 실행되고, `@Transactional`이 없어도 롤백되기 때문입니다.

하지만 `@Transactional`을 사용하지 않고 `@AfterEach`에서 명시적으로 DB를 정리하면,  
**`OrderService` 내부에 `@Transactional`이 없는 경우 예상치 못한 동작을 발견할 수 있습니다.**

---

### 📌 문제1 : `OrderService`에 `@Transactional`이 없는 경우
```java
@Service
public class OrderService {
...
```
위와 같은 코드에서 `OrderService`에 `@Transactional`이 없다면,
- `orderRepository.save(order);`가 실행되고
- 예외가 발생하지만 **DB에는 주문 데이터가 남아 있음**
- `@Transactional`이 있었다면 예외 발생 시 롤백되어야 하지만, 현재는 데이터가 남아 있음

이런 문제를 `@AfterEach`를 사용하면 감지할 수 있음 → 데이터가 남아 있어 다른 테스트에 영향을 주기 때문

---

### 📌 문제2: `OrderService`에 `@Transactional`이 있는 경우
```java
@Service
@Transactional
public class OrderService {
...
```
- `@Transactional`이 추가되면 예외가 발생할 경우 **DB에 저장된 데이터도 롤백됨**
- 따라서 `@AfterEach`에서 데이터를 삭제할 필요 없이, **테스트 자체가 독립적으로 유지됨**

---

## 3. **결론**
✅ `@Transactional`을 테스트 클래스에 추가하면 자동 롤백되어 편리하지만,  
✅ `@AfterEach`를 사용하면 `OrderService` 내부의 `@Transactional`이 정상적으로 동작하는지 검증할 수 있음  
✅ `@Transactional`이 없어서 DB에 데이터가 남는다면, `@AfterEach`에서 삭제하면서 문제를 감지할 수 있음

---

### 🚀 **추천하는 테스트 방법**
- `@Transactional`을 붙이면 **편리하게 롤백 가능**
- 하지만 서비스 로직(`OrderService`) 내부에서 트랜잭션이 잘 적용되었는지 확인하고 싶다면 `@AfterEach`를 사용하여 직접 정리하는 것도 좋은 방법
